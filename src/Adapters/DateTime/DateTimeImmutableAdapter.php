...
final class DateTimeImmutableAdapter implements AdapterInterface {
    ...
    function deserialize(DataMapper $mapper, mixed $data): DateTimeImmutable {
        ...
        $dateTime = DateTimeImmutable::createFromFormat($this->format, $data, $this->timeZone);
        if ($dateTime === false) {
            throw new DeserializeException(
    ...
    function serialize(DataMapper $mapper, mixed $data): string {
        if (!($data instanceof DateTimeImmutable)) {
            throw new SerializeException(
                StringHelper::interpolate("Expected an instance of DateTimeImmutable, but received: ??", $data));
        }
